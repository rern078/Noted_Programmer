...
(
    ...
    CONSTRAINT emp_sr3_emp_id_pk PRIMARY KEY(emp_id),
    --Talbe Level
...
(
    ...
    oDate DATE,
    emp_id NUMBER (5)REFERENCES emp_sr2(emp_id)
    --Column Level
);
...
DROP TABLE employee_sr3
PURGE;
--DELETE NOT IN RECYCLEBIN
...
--8
FLASHBACK TABLE types TO BEFORE
DROP;
FLASHBACK TABLE products TO BEFORE
DROP;
...
CREATE VIEW dept10_sr1
AS
    SELECT *
    FROM employees
    WHERE department_id=10;

CREATE VIEW dept20_sr1
AS
    SELECT *
    FROM employees
    WHERE department_id=20;

CREATE VIEW dept50_sr1
AS
    SELECT *
    FROM employees
    WHERE department_id=50;

CREATE VIEW dept60_sr1
AS
    SELECT *
    FROM employees
    WHERE department_id=60;

CREATE VIEW dept90_sr1
AS
    SELECT *
    FROM employees
    WHERE department_id=90;
...
ADD CONSTRAINT emp2_sr1_id_pk PRIMARY KEY(id);
--Table Level Constraint
...
SET
CONSTRAINT emp3_sr1_id_pk IMMEDIATE;

SET
CONSTRAINT emp3_sr1_id_pk DEFERRED;
...
(
    ...
    CONSTRAINT emp4_sr1_id_name_uk UNIQUE(id,name)
...
CREATE TABLE emp5_sr1(
    ...
    USING INDEX
(
CREATE INDEX emp5_sr1_id_idx ON emp5_sr1(id)
),
    name VARCHAR2
(25)  
);
...
FLASHBACK TABLE staff_new TO BEFORE
DROP;
...
ALTER TABLE staff_new
MODIFY manager_id REFERENCES staff_new
(sid);
...
FLASHBACK TABLE emp_sr1 TO BEFORE
DROP;
...
INSERT INTO product_sr1
    (pName, qtyInStock, price)
...
(
    ...
    --3
    FLASHBACK TABLE staff_demo TO BEFORE
    DROP;
...
USING (region_id) 
        ...
        CREATE VIEW countries_sr1_Europe
        AS
            SELECT *
            FROM countries_sr1
            WHERE region_id=(SELECT region_id
...
WITH CHECK
        ...
        INSERT INTO emp6_sr1
        VALUES(3, 'MAKARA', DEFAULT);
        --Explicat Default View
...
WHERE id=1;
    ...
    INTO
        ...
        (i, h, s)
    INTO mgr_history
        (employee_id, manager_id, salary,hire_date) VALUES
        (i,m,s,h)
...
WHEN
        hire_date<'01-JAN-98'
        THEN     --DD-MON-RR
    INTO emp_history VALUES
        (employee_id, hire_date, salary)
WHEN commission_pct IS NOT NULL THEN
    INTO emp_sales VALUES
...
WHEN commission_pct IS NOT NULL THEN
    ...
    INTO
        emp_sales_info
        VALUES(id, week_id, qty_mon, 1)
        INTO emp_sales_info VALUES
        (id, week_id, qty_tue, 2)
    INTO emp_sales_info VALUES
        (id, week_id, qty_wed, 3)
    INTO emp_sales_info VALUES
        (id, week_id, qty_thu, 4)
    INTO emp_sales_info VALUES
        (id, week_id, qty_fri, 5)
        SELECT *
...
WHERE employee_id=174;
        ...
        SELECT salary, VERSIONS_STARTTIME, VERSIONS_ENDTIME
        --Date
        FROM emp10_sr1
VERSIONS
        BETWEEN SCN MINVALUE AND MAXVALUE              --Old Values
WHERE employee_id=174;
...
TO chamrern;
                        ...
                        (
                            ...
                            JOB_ID VARCHAR2
                            (10) CONSTRAINT EMP_JOBS_FK REFERENCES JOBS_SR1
                            (JOB_ID),
    SALARY NUMBER
                            (8,2),
    MANAGER_ID NUMBER
